    ...
    if (v.children().hasClass('sc-mark-show')) {
        ...
        for (var i = 0; i < content.length; i++) {
            matterHtml += '<li class="announceItem"><div><div class="fl announceImg">' +
                '<img class=" " src="/xmzhgl_xzb/images/content.png"/></div>' +
 ...
 * 通过项目代码查询项目日志信息
 */
function loadXmrzByXmdm(xmdm, xmbz) {
    $.postJSON("/xmzhgl_xzb/xmzl/loadXmrzByXmdm", {
        "xmdm": xmdm
    }, function(data) {
        if (data && data[0] == "Y") {
            //获得项目工程详情
            ...
            if (xmrzList && xmrzList.length > 0) {
                ...
                for (var i = 0; i < xmrzList.length; i++) {
                    ...
                    //添加标记
                    markNew[rzsj] = [{ title: getStrNoNull(xmrzList[i][1]), startTime: getFormatDate(xmrzList[i][0]) + '</br>' + str, endTime: '<a  href="javascript:void(0);" class="ckxq_btn" onclick="showRzxq(\'' + xmdm + '\',\'' + rzsj + '\',\'' + xmbz + '\')" style="float: right">查看详情>></a>' + '</a>' }];
                }
                initDate(markNew);
            }
        }
 ...
 * 跳转到项目日志详情页面
 */
function showRzxq(xmdm, rzsj, xmbz) {
    window.top.location.href = '/xmzhgl_xzb/show/xmzl/projectDateCkxq?xmdm=' + xmdm + '&rzsj=' + rzsj + '&xmbz=' + xmbz + '&tid=' + Math.round(Math.random() * 1000000);
}