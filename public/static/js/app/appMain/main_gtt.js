 ...
 * 加载项目跟踪列表
 */
function loadXmgzList() {
	$.postJSON("/xmzhgl_xzb/app/xmjindu/loadXmgzList", {"xmdm":xmdm},function (data, textStatus) {
		if(data){
			var list=data[0];
			var lis = [];
			lis.push('<tr style="border: 1px dashed #2e5280;">');
			lis.push('<th style="width: 70px;text-align:center">序号</th>');
			lis.push('<th style="width: 100px;text-align:center">任务名称</th>');
			...
			lis.push('<th style="width: 200px;text-align:center">主要负责人</th>');
			lis.push('<th style="width: 400px;text-align:center">进度</th>');
			lis.push('<th style="text-align:center">存在问题</th>');
			lis.push('<th style="width: 140px;text-align:center">需要协调的经办人</th>');
			...
			if (list && list.length > 0) {
				...
				lis.push('<tr><td colspan="11">无数据</td></tr>');
			}
			$("#xmgzckTable").html(lis.join(""));
		}
	});
...
//	    gantt.config.grid_width = 200;//左侧表宽
...
//		];
	...
	$.postJSON("/xmzhgl_xzb/app/xmjindu/loadXmGantt", {"xmdm":xmdm},function (data, textStatus) {
		if(data[0].length>0){
			var p = "";
			if(data[0].length>0){
				var list = data[0];
